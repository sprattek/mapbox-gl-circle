...
class MapboxCircle {
     ...
     * @private
     */
    _onCenterHandleMouseDown(event) {
        if (this._getCursorStyle() !== 'move' || event.originalEvent.button !== 0) {
    ...
    _onRadiusHandlesMouseDown(event) {
        if (!this._getCursorStyle().endsWith('-resize') || event.originalEvent.button !== 0) {
